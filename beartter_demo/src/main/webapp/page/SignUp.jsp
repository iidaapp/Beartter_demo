...
<body>
	<div class="container" align="center">
		<div class="form-horizontal">
			...
			<form action="confirm" method="post" role="form" class="form-horizontal">

				<div class="form-group">
					<label for="inputUserName">Beartterユーザー名</label> <input class="form-control" id="inputUserName"
				...
				<div class="form-group">
					<label for="inputEmailAddress">メールアドレス<br /></label> <input type="text" name="mailAddress"
						class="form-control" size="200" id="inputEmailAddress"
						<c:if test="${!empty signUpForm}">value="${signUpForm.mailAddress}"</c:if>>
				</div>

				<div class="form-inline form-group">
					<div class="form-group">
						<label for="birth_day">誕生日</label> <input type="text" name="year" size="4"
							class="form-control" <c:if test="${!empty signUpForm}">value="${signUpForm.year}"</c:if>>年
						<input type="text" name="month" size="2" class="form-control"
							<c:if test="${!empty signUpForm}">value="${signUpForm.month}"</c:if>>月 <input
							type="text" name="day" size="2" class="form-control"
				...
				パスワード不一致
					<div class="form-group has-error">
						<label for="password" class="control-label">パスワード</label> <input type="password"
				...
				<c:if test="${empty SamePassword or SamePassword}">
					...
					<div class="form-group">
						<label for="passwordretry">パスワード確認</label> <input type="password" name="passwordConfirm"
							class="form-control" size="45">
					</div>
				</c:if>
